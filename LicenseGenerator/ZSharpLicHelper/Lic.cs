...
using ZSharpLicHelper.Helper;
using ZSharpLicHelper.APIEntities;
...
namespace ZSharpLicHelper
{
    public class Lic
    {
        ...
        {
            if (!String.IsNullOrWhiteSpace(ZSharpLicHelper.Helper.GenHelper.LicReadRegistry("AppLoginAccess")))
            {
                return Convert.ToBoolean(ZSharpLicHelper.Helper.GenHelper.LicReadRegistry("AppLoginAccess"));
            }
            else
                return false;
        }
        public bool ApplicationStatus(string licFile)
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    string macAddress = GetMACAddress();
                    string cpuId = ZSharpLicHelper.Helper.GenHelper.cpuId();
                    string motherBoardId = ZSharpLicHelper.Helper.GenHelper.motherBoardId();
                    string diskId = ZSharpLicHelper.Helper.GenHelper.diskId();
                    // MessageBox.Show(macAddress);
                    ...
                    bool isuserLogin = IsUserLogin();
                    if (_ds.Tables[0].Rows[0]["MacAddress"].ToString().Trim().Equals(macAddress.Trim(), StringComparison.InvariantCultureIgnoreCase) && _ds.Tables[0].Rows[0]["MachineName"].ToString().Equals(Environment.MachineName.Trim(), StringComparison.InvariantCultureIgnoreCase) && _ds.Tables[0].Rows[0]["CPUId"].ToString().Equals(cpuId, StringComparison.InvariantCultureIgnoreCase) && _ds.Tables[0].Rows[0]["MotherBoardId"].ToString().Equals(motherBoardId, StringComparison.InvariantCultureIgnoreCase) && _ds.Tables[0].Rows[0]["DiskId"].ToString().Equals(diskId, StringComparison.InvariantCultureIgnoreCase) && isuserLogin)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            Global.variable.app_lic_status = false;
                            Global.variable.app_lic_status_mess = "User is not logged in with System!" + "\nwww.zcodia.com.au";
                            Appstatus("NOAccess");
                            Appmessage("Please Login on System with DAMProFileName Creator!" + "\nwww.zcodia.com.au");
                        }
                    }
                }
            }
            else
            {
                ...
                Appstatus("NOAccess");
                Appmessage("License file is not available. Please Contact Administartor or login with LicensingSDKLogin application!");
            }
